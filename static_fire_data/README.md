...
| **BALLSStick (2″ subscale)** | high | **`raw/ballsstick_subscale_raw.csv`** | 417 | ⚠️ **RAW — blocked on pressure-channel provenance** | Firing ≈2.31 s at raw t≈1388.4 (DAQ power-on clock). **No quiescent baseline** (PSI ramps monotonically from sample 1 while volts stay flat → PSI isn't a linear map of the logged volts). **Do not apply a constant offset.** Load-cell impulse 0.85× expected. **No 3″-scale firing exists**; needs justified 2″→3″ scaling. See `raw/ballsstick_subscale_raw.notes.md`. |
